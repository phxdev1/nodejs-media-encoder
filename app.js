...
watcher.on('add', file_path => {
    ...
    getYouTubePromo(uploaded_file, file_name, SCREEN_WIDTH_1080, "1080").run();
});
...
function getYouTubePromo(uploaded_file, file_name, _screen_width, title){
    ...
    filterChain.push({
        filter: 'overlay', options: null,
        inputs: ['0', 'ckout'], outputs: 'overlay_out'
    });
    filterChain.push({
        filter: 'scale', options: '3840:-1',
        inputs: 'overlay_out', outputs: 'scaled'
    });
    filterChain.push({
        filter: 'concat', options: "n=2:v=1:a=1",
        inputs: ['scaled', '0:a','2', '2:a'], outputs: ['youtube_promo_vid', 'youtube_promo_audio']
    });
